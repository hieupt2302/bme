...
export default function PatientDetailPage() {
  ...
  const chart = sessions.map(s=>({ date: new Date(s.created_at).toLocaleDateString('vi-VN',{day:'2-digit',month:'2-digit'}), angle: +(s.avg_angle_deg||0).toFixed(1), tremor: +(s.avg_tremor||0).toFixed(2) }));
  ...
  return (<>
    ...
    <div className="bg-surface-container rounded-xl p-6 mb-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
    ...
    {chart.length > 1 && (
      <div className="bg-surface-container p-6 rounded-2xl shadow-xl mb-8">
        <h3 className="font-bold text-on-surface mb-4">Tiến trình phục hồi theo thời gian</h3>
